    ...
    const renderPests = () => {
        ...
        return (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gap: '2.5rem' }}>
                {(crop.pests_diseases || []).map((item, idx) => (
                    <motion.div
                        key={idx}
                        className="card"
                        whileHover={{ y: -10 }}
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
                            <div style={{ background: 'var(--primary-light)', padding: '0.75rem', borderRadius: '12px' }}>
                                {item.name.toLowerCase().includes('bacteria') || item.name.toLowerCase().includes('virus') ? <Microscope size={24} color="var(--primary)" /> : <Bug size={24} color="var(--primary)" />}
                            </div>
                            <h4 style={{ fontSize: '1.5rem', fontWeight: 900 }}>{item.name}</h4>
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                            <div style={{ display: 'flex', gap: '1rem' }}>
                                ...
                                <div>
                                    <span style={{ fontWeight: 800, fontSize: '0.9rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Symptoms</span>
                                    <p style={{ fontSize: '0.95rem', marginTop: '0.25rem' }}>{item.symptoms}</p>
                                </div>
                            </div>
                            <div style={{ display: 'flex', gap: '1rem' }}>
                                <div style={{ color: 'var(--primary)', marginTop: '0.2rem' }}><Clock size={18} /></div>
                                <div>
                                    <span style={{ fontWeight: 800, fontSize: '0.9rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Risk Stage</span>
                                    <p style={{ fontSize: '0.95rem', marginTop: '0.25rem' }}>{item.risk_stage}</p>
                                </div>
                            </div>
                            <div style={{ display: 'flex', gap: '1rem' }}>
                                <div style={{ color: 'var(--primary)', marginTop: '0.2rem' }}><ShieldCheck size={18} /></div>
                                <div>
                                    <span style={{ fontWeight: 800, fontSize: '0.9rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Prevention</span>
                                    <p style={{ fontSize: '0.95rem', marginTop: '0.25rem' }}>{item.prevention}</p>
                                </div>
                            </div>
                            <div style={{ display: 'flex', gap: '1rem', background: 'var(--primary-light)', padding: '1.25rem', borderRadius: '16px' }}>
                                <div style={{ color: 'var(--primary)', marginTop: '0.2rem' }}><Stethoscope size={20} /></div>
                                <div>
                                    <span style={{ fontWeight: 900, fontSize: '0.9rem', color: 'var(--primary)', textTransform: 'uppercase' }}>Treatment Plan</span>
                                    <p style={{ fontSize: '0.95rem', fontWeight: 700, marginTop: '0.25rem' }}>{item.treatment}</p>
                                </div>
                            </div>
                        </div>
                    </motion.div>
