            ...
            <header style={{ marginBottom: 'var(--space-8)', textAlign: 'center' }}>
                ...
                >
                    ...
                    <div style={{ maxWidth: '600px', margin: '0 auto', position: 'relative' }}>
                        ...
                        <input
                            type="text"
                            placeholder={marketMode === 'crop' ? "Search commodities..." : "Search resources..."}
                            value={searchTerm}
                ...
                {marketMode === 'crop' ? (
                                        ...
                                        filteredMarket.map((item, i) => (
                                            ...
                                            >
                                                ...
                                                <div style={{ textAlign: 'center' }}>
                                                    ...
                                                    <div style={{ fontWeight: 900, fontSize: '1.4rem', color: 'var(--text)' }}>{item.price.split('/')[0]} <span style={{ fontSize: '0.85rem', fontWeight: 500, color: 'var(--text-muted)' }}>/ {item.price.split('/')[1] || 'unit'}</span></div>
                                                </div>

                                                <div style={{ height: '40px', display: 'flex', alignItems: 'center' }}>
                                                    <svg width="120" height="30" style={{ opacity: 0.6 }}>
                                                        <path
                                                            d={item.trend && item.trend.startsWith('+') ? "M0 25 L20 20 L40 22 L60 10 L80 15 L100 5 L120 8" : "M0 5 L20 15 L40 10 L60 22 L80 18 L100 25 L120 23"}
                                                            fill="none"
                                                            stroke={item.trend && item.trend.startsWith('+') ? "#00b894" : "#ff7675"}
                                                            strokeWidth="3"
                                                            strokeLinecap="round"
                                                        />
                                                    </svg>
                                                ...
                                                <div style={{ textAlign: 'right' }}>
                                                    <div style={{
                                                        ...
                                                        fontSize: '0.9rem',
                                                        fontWeight: 800,
                                                        background: item.trend && item.trend.startsWith('+') ? '#e6fcf5' : '#fff5f5',
                                                        color: item.trend && item.trend.startsWith('+') ? '#00b894' : '#ff7675'
                                                    }}>
                                                        {item.trend || 'Stable'} {item.trend && item.trend.startsWith('+') ? <TrendingUp size={16} /> : <TrendingDown size={16} />}
                                                    </div>
                                                </div>
                                            </motion.div>
                                        ))
                    ...
                    >
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '2.5rem' }}>
                            {resourceData.filter(r => category === 'All' || r.category === category).map((res, i) => (
                                <motion.div
                                ...
                                >
                                    <div style={{
                                        ...
                                        {getProductIcon(res.product, res.category, 64)}
                                    </div>
                ...
                {selectedItem && (
                    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
                        ...
                        >
                            <button
                                onClick={() => setSelectedItem(null)}
                                style={{ position: 'absolute', right: '2.5rem', top: '2.5rem', background: '#f8fafc', border: 'none', padding: '1rem', borderRadius: '50%', cursor: 'pointer', transition: 'all 0.2s' }}
                            ...
                            <div style={{ display: 'flex', gap: '3rem', marginBottom: '3.5rem' }}>
                                <div style={{ width: '140px', height: '140px', background: 'var(--primary-light)', borderRadius: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                    {getProductIcon(selectedItem.product, selectedItem.category, 70)}
                                </div>
                                <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                                    <span style={{ fontSize: '0.85rem', fontWeight: 900, color: 'var(--primary)', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '0.75rem', display: 'block' }}>{selectedItem.category}</span>
                                    <h3 style={{ fontSize: '2.8rem', fontWeight: 900, margin: '0 0 1rem', letterSpacing: '-1px', lineHeight: 1 }}>{selectedItem.product}</h3>

                                    {selectedItem.price && (
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                                            <div style={{ fontSize: '2rem', fontWeight: 900, color: 'var(--primary)' }}>
                                                {selectedItem.price.split('/')[0]} <span style={{ fontSize: '1rem', fontWeight: 500, color: 'var(--text-muted)' }}>/ {selectedItem.price.split('/')[1]}</span>
                                            </div>
                                            {selectedItem.trend && (
                                                <div style={{
                                                    display: 'inline-flex',
                            ...
                            <div style={{ background: '#f8fafc', padding: '2.5rem', borderRadius: '32px', marginBottom: '3rem' }}>
                                ...
                                <p style={{ lineHeight: 1.8, color: 'var(--text-muted)', fontSize: '1.1rem', fontWeight: 500 }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                                <button className="btn btn-primary" style={{ padding: '1.5rem', borderRadius: '20px', fontSize: '1.1rem', fontWeight: 800 }}>
                                    Contact Supplier <ArrowRight size={22} />
                                </button>
                                <button className="btn btn-secondary" style={{ padding: '1.5rem', borderRadius: '20px', fontSize: '1.1rem', fontWeight: 800 }}>
                                    Download Specs <ExternalLink size={20} />
                                </button>
                            </div>
                        </motion.div>
                    </div>
